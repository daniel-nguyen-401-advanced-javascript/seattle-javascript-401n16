...
When discussing Linked Lists, we often refer to the first `Node` in the list as the `head`, and the last `Node` as the `tail`.  The most common operation upon a Linked List is a *traversal*: moving from the `head` to the `tail`. 
